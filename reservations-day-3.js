document.addEventListener('DOMContentLoaded', () => {
    ...
    const addReservationBtn = document.getElementById('addReservationBtn'); // Button in header
    ...
    if (savedTheme) {
        ...
        applyTheme('dark'); // Default to dark mode if no preference saved
    }
    ...
    // --- Modal Functions ---
    function openModal(reservation = null, suggestedTableId = null, suggestedTime = null) {
        reservationModal.classList.add('visible');
        reservationModalOverlay.classList.add('visible');
    ...
    function closeModal() {
        ...
        reservationModalOverlay.classList.remove('visible');
        ...
        filteredTables.forEach(table => {
            ...
            timeSlots.forEach((slotTime, index) => {
                ...
                if (reservationToDisplay) {
                    ...
                    // Add click listener to open modal for adding new reservation
                    timeSlotCell.addEventListener('click', () => {
                        openModal(null, table.id, slotTime); // Pass table ID and time for pre-filling
                    });
    ...
    reservationModalOverlay.addEventListener('click', (event) => {
        if (event.target === reservationModalOverlay) { // Only close if overlay itself is clicked, not modal content
            closeModal();
        }
    });
